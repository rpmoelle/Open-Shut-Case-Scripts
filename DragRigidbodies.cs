...
public class DragRigidbodies : MonoBehaviour {
    //Change this to work with new design controls
	public float speed = 15;

    public Quaternion originalOrientation;
    bool needToTurn;
    public bool movingAStuckNote = false;
	...
	void Start () {
		...
		startSpeed = speed;
        originalOrientation = GameObject.Find("document1").transform.rotation;
	}
	...
	void Update () {
        ...
        if (!gameObject.GetComponent<Dialogue> ().running) {
			...
			if (Physics.Raycast (ray, out hit, 6) && hit.collider.gameObject.GetComponent ("Rigidbody") != null) {
				canPickUp = true;
               
                if (hit.collider.gameObject.tag == "paper")
                {
                    GameObject paper = hit.collider.gameObject;
                  
                    if (paper.GetComponent<freezeRot>().isStuck)
                    {
                        paper.GetComponent<freezeRot>().isStuck = false;
                        paper.GetComponent<Rigidbody>().isKinematic = false;
                    }
                    else if(paper.gameObject.transform.localRotation.x > 90f)
                    {
                        needToTurn = true;
                    }
				...
				if (Physics.Raycast (ray, out hit, 6)) {
					if (hit.collider.GetComponent<Rigidbody> () != null) {
                            if (hit.collider.gameObject.tag == "paper")
                            {
                                ...
                                if (hit.collider.gameObject.GetComponent<freezeRot>().isStuck)
                                {
                                ...
                                if (needToTurn)
                            {
                                hit.collider.gameObject.transform.localRotation = originalOrientation;
                            }
